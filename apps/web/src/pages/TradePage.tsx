...
export default function TradePage() {
  ...
  const actionBody = useMemo(() => {
    ...
    if (action.kind === 'cancel') return <>Cancel <b>{action.order.symbol}</b> order{action.childOrderIds.length ? <> together with <b>{action.childOrderIds.length} linked SL/TP</b></> : null}?</>;
    if (action.kind === 'cancelAll') return <>Cancel <b>all {action.symbol}</b> orders on <b>{action.accountName}</b>?</>;
    if (action.kind === 'close') return <>Close <b>{action.percent}%</b> of <b>{action.position.symbol} {action.position.side}</b> at Market with reduce-only?</>;
    return <>Cancel all <b>{action.position.symbol}</b> orders and close the entire <b>{action.position.side}</b> position at Market?</>;
  }, [action]);

  return (
    <div className="page">
      <div className="page-head">
        <div><h1>Trade Control</h1><p>Real-time positions, orders and fills across exchange accounts</p></div>
        <div className="top-controls">
          ...
          <input className="input" placeholder="Symbol filter" value={symbol} onChange={(e) => setSymbol(e.target.value.toUpperCase())} />
          <button className="btn secondary" onClick={refresh}><RefreshCw size={14} />Refresh</button>
          {!live && <span className="badge demo">actions locked</span>}
        </div>
      </div>
      ...
      <div className="account-cards">
        {summary.data?.map((s) => (
          <div className={account === s.accountId ? 'account-card card active' : 'account-card card'} key={s.accountId} onClick={() => setAccount(account === s.accountId ? 0 : s.accountId)}>
            <h3>{s.accountName || `Account ${s.accountId}`} {s.online ? <span className="badge live">online</span> : <span className="badge">offline</span>}</h3>
            <div className="account-stat"><span>Equity</span><b>{s.online ? money(s.equity) : '—'}</b></div>
            <div className="account-stat"><span>Unrealized</span><b className={s.unrealisedPnl >= 0 ? 'positive' : 'negative'}>{s.online ? money(s.unrealisedPnl) : '—'}</b></div>
            <div className="account-stat"><span>Positions / Orders</span><b>{s.online ? `${s.positions} / ${accountGroupedCount(s.accountId)}` : '—'}</b></div>
          </div>
        ))}
      </div>

      <div className="tabs">{tabs.map(([id, label, count]) => <button key={id} className={tab === id ? 'tab active' : 'tab'} onClick={() => setTab(id)}>{label} {count}</button>)}</div>

      <div className="card table-card">
        {tab === 'positions' && <table className="data-table"><thead><tr><th>Account</th><th>Symbol</th><th>Side</th><th>Size</th><th>Entry</th><th>Mark</th><th>uPnL</th><th>SL</th><th>TP</th><th>Liq</th><th>Actions</th></tr></thead><tbody>
        ...
        {tab === 'orders' && <table className="data-table"><thead><tr><th>Account</th><th>Symbol</th><th>Side</th><th>Type</th><th>Status</th><th>Price</th><th>Qty</th><th>Filled</th><th>Trigger</th><th>SL</th><th>TP</th><th /></tr></thead><tbody>
          {visibleGroupedOrders.map((group) => {
            ...
            return <tr key={`${o.accountId}-${o.orderId}`} title={group.children.length ? `${group.children.length} protective order(s) grouped under this order` : undefined}>
              <td>{o.accountName}</td><td><b>{o.symbol}</b>{group.children.length ? <span className="badge order-child-badge"> +{group.children.length} SL/TP</span> : null}</td><td className={o.side === 'Buy' ? 'positive' : 'negative'}>{o.side}</td><td>{o.orderType}</td><td>{o.orderStatus}</td><td>{num(o.price || o.triggerPrice || 0, 6)}</td><td>{num(o.qty, 6)}</td><td>{num(o.cumExecQty, 6)}</td><td>{o.triggerPrice && o.price ? num(o.triggerPrice, 6) : '—'}</td><td>{slPrice ? num(slPrice, 6) : '—'}</td><td>{tpPrice ? num(tpPrice, 6) : '—'}</td>
              <td><div className="row-actions"><button className="mini-btn danger" disabled={!live} onClick={() => setAction({ kind: 'cancel', order: o, childOrderIds: group.children.map((child) => child.orderId) })}>Cancel</button><button className="mini-btn" disabled={!live} onClick={() => setAction({ kind: 'cancelAll', accountId: o.accountId, accountName: o.accountName, symbol: o.symbol })}>Cancel all</button></div></td>
            </tr>;
          })}
        </tbody></table>}

        {tab === 'executions' && <table className="data-table"><thead><tr><th>Account</th><th>Time</th><th>Symbol</th><th>Side</th><th>Price</th><th>Qty</th><th>Fee</th><th>Order ID</th></tr></thead><tbody>{filter(executions.data).map((x) => <tr key={`${x.accountId}-${x.execId}`}><td>{x.accountName}</td><td>{new Date(x.execTime).toLocaleString()}</td><td><b>{x.symbol}</b></td><td>{x.side}</td><td>{num(x.execPrice, 6)}</td><td>{num(x.execQty, 6)}</td><td>{num(x.execFee, 6)}</td><td>{x.orderId}</td></tr>)}</tbody></table>}

        {tab === 'history' && <table className="data-table"><thead><tr><th>Account</th><th>Time</th><th>Symbol</th><th>Side</th><th>Type</th><th>Status</th><th>Avg / Price</th><th>Qty</th><th>Filled</th></tr></thead><tbody>{filter(history.data).map((o) => <tr key={`${o.accountId}-${o.orderId}`}><td>{o.accountName}</td><td>{new Date(o.updatedTime).toLocaleString()}</td><td><b>{o.symbol}</b></td><td>{o.side}</td><td>{o.orderType}</td><td>{o.orderStatus}</td><td>{num(o.price, 6)}</td><td>{num(o.qty, 6)}</td><td>{num(o.cumExecQty, 6)}</td></tr>)}</tbody></table>}

        {((tab === 'positions' && !filter(positions.data).length) || (tab === 'orders' && !visibleGroupedOrders.length) || (tab === 'executions' && !filter(executions.data).length) || (tab === 'history' && !filter(history.data).length)) && <div className="empty">No data for the selected filters.</div>}
      </div>

      {selected && <aside className="drawer" style={{ position: 'fixed', right: 14, top: 14, bottom: 14, zIndex: 60, maxHeight: 'none' }}>
        <div className="drawer-head"><div><h2>{selected.symbol} {selected.side}</h2><div className="muted">{selected.accountName} · size {num(selected.size, 6)}</div></div><button className="icon-btn" onClick={() => setSelected(null)}><X size={16} /></button></div>
        <div className="metric-grid"><div className="metric"><small>Entry</small><strong>{num(selected.avgPrice, 6)}</strong></div><div className="metric"><small>Mark</small><strong>{num(selected.markPrice, 6)}</strong></div><div className="metric"><small>uPnL</small><strong className={selected.unrealisedPnl >= 0 ? 'positive' : 'negative'}>{money(selected.unrealisedPnl)}</strong></div><div className="metric"><small>Liquidation</small><strong>{selected.liqPrice ? num(selected.liqPrice, 6) : '—'}</strong></div></div>
        <div className="drawer-section"><div className="field"><label>Stop Loss</label><input className="input" type="number" step="any" value={sl} onChange={(e) => setSl(e.target.value)} /></div><div className="field"><label>Take Profit</label><input className="input" type="number" step="any" value={tp} onChange={(e) => setTp(e.target.value)} /></div><div className="field"><label>Trailing Stop distance</label><input className="input" type="number" step="any" value={trailing} onChange={(e) => setTrailing(e.target.value)} /></div><div className="drawer-actions"><button className="btn secondary" disabled={!live || updateStops.isPending} onClick={() => updateStops.mutate({})}>Update SL / TP / Trail</button><button className="btn secondary" disabled={!live || updateStops.isPending} onClick={() => updateStops.mutate({ stopLoss: selected.avgPrice })}>Move SL to breakeven</button><button className="btn ghost" onClick={() => { location.href = `/?symbol=${encodeURIComponent(selected.symbol)}`; }}>Open on chart</button></div></div>
        <div className="drawer-section"><h3 style={{ fontSize: 12 }}><AlertTriangle size={14} /> Emergency</h3><div className="drawer-actions"><button className="btn warning" disabled={!live} onClick={() => setAction({ kind: 'close', position: selected, percent: 50 })}>Close 50%</button><button className="btn danger" disabled={!live} onClick={() => setAction({ kind: 'close', position: selected, percent: 100 })}>Force close 100%</button><button className="btn danger" disabled={!live} onClick={() => setAction({ kind: 'flatten', position: selected })}>Flatten symbol</button></div></div>
      </aside>}

      <ConfirmDialog
        open={Boolean(action)}
        title={action?.kind === 'flatten' ? 'Emergency flatten' : action?.kind === 'close' ? 'Close position' : action?.kind === 'cancelAll' ? 'Cancel all orders' : 'Cancel order'}
        body={actionBody}
        danger
        onClose={() => setAction(null)}
        onConfirm={() => { if (action) run.mutate(action); }}
        confirmLabel={action?.kind === 'flatten' ? 'FLATTEN NOW' : action?.kind === 'close' ? 'Close position' : action?.kind === 'cancelAll' ? 'Cancel all' : 'Cancel order'}
      />
    </div>
  );
}
