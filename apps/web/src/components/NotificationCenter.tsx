...
type NotificationRow={
  id:number;category:'market'|'trading'|'system';eventType:string;severity:string;title:string;message:string;accountName:string|null;symbol:string|null;actionUrl:string|null;readAt:string|null;createdAt:string;
};
...
export default function NotificationCenter(){
  const [open,setOpen]=useState(false);const qc=useQueryClient();const navigate=useNavigate();
  ...
  return <div className="notification-center">
    <button className={open?'notification-bell open':'notification-bell'} onClick={()=>setOpen(v=>!v)} title="Notifications" aria-label="Notifications">
    ...
    {open&&<div className="notification-popover card">
      <div className="notification-head"><div><strong>Notifications</strong><small>{unread} unread</small></div><div className="notification-head-actions"><button className="mini-btn" disabled={!unread||readAll.isPending} onClick={()=>readAll.mutate()}><CheckCheck size={13}/> Read all</button><button className="icon-btn notification-close" onClick={()=>setOpen(false)}><X size={14}/></button></div></div>
      <div className="notification-list">
        {rows.isLoading&&<div className="notification-empty">Loading…</div>}
        {!rows.isLoading&&!rows.data?.length&&<div className="notification-empty">No notifications yet.</div>}
        {rows.data?.map(row=><button key={row.id} className={row.readAt?'notification-row':'notification-row unread'} onClick={()=>go(row)}>
          <span className={`notification-icon ${row.category} ${row.severity}`}><Icon row={row}/></span>
          <span className="notification-copy"><span className="notification-title">{row.title}</span><span className="notification-message">{row.message}</span><span className="notification-meta">{[row.accountName,row.symbol,when(row.createdAt)].filter(Boolean).join(' · ')}</span></span>
